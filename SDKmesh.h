...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    void WriteSDKMeshToTextFile(const SDKMESH_HEADER& header,
        ...
        const SDKMESH_INDEX_BUFFER_HEADER* indexBufferHeaders,
        const SDKMESH_MESH* meshes,
        const SDKMESH_SUBSET* subsets,
        const SDKMESH_FRAME* frames,
        const SDKMESH_MATERIAL* materials,
        const std::string& filename)
    {
        ...
        // Write SDKMESH_HEADER
        file << "SDKMESH_HEADER:\n";
        file << "Version: " << header.Version << "\n";
        file << "IsBigEndian: " << static_cast<int>(header.IsBigEndian) << "\n";
        file << "HeaderSize: " << header.HeaderSize << "\n";
        file << "NonBufferDataSize: " << header.NonBufferDataSize << "\n";
        file << "BufferDataSize: " << header.BufferDataSize << "\n";
        file << "NumVertexBuffers: " << header.NumVertexBuffers << "\n";
        file << "NumIndexBuffers: " << header.NumIndexBuffers << "\n";
        file << "NumMeshes: " << header.NumMeshes << "\n";
        file << "NumTotalSubsets: " << header.NumTotalSubsets << "\n";
        file << "NumFrames: " << header.NumFrames << "\n";
        file << "NumMaterials: " << header.NumMaterials << "\n\n";

        // Write vertex buffers
        for (UINT i = 0; i < header.NumVertexBuffers; ++i) {
            file << "SDKMESH_VERTEX_BUFFER_HEADER [" << i << "]:\n";
            file << "NumVertices: " << vertexBufferHeaders[i].NumVertices << "\n";
            file << "SizeBytes: " << vertexBufferHeaders[i].SizeBytes << "\n";
            file << "StrideBytes: " << vertexBufferHeaders[i].StrideBytes << "\n";
            // Here, D3DVERTEXELEMENT9 structure printing can be customized
            file << "DataOffset: " << vertexBufferHeaders[i].DataOffset << "\n\n";
        }

        // Write index buffers
        for (UINT i = 0; i < header.NumIndexBuffers; ++i) {
            file << "SDKMESH_INDEX_BUFFER_HEADER [" << i << "]:\n";
            file << "NumIndices: " << indexBufferHeaders[i].NumIndices << "\n";
            file << "SizeBytes: " << indexBufferHeaders[i].SizeBytes << "\n";
            file << "IndexType: " << indexBufferHeaders[i].IndexType << "\n";
            file << "DataOffset: " << indexBufferHeaders[i].DataOffset << "\n\n";
        }

        // Write meshes
        for (UINT i = 0; i < header.NumMeshes; ++i) {
            file << "SDKMESH_MESH [" << i << "]:\n";
            file << "Name: " << meshes[i].Name << "\n";
            file << "NumVertexBuffers: " << static_cast<int>(meshes[i].NumVertexBuffers) << "\n";
            file << "IndexBuffer: " << meshes[i].IndexBuffer << "\n";
            file << "NumSubsets: " << meshes[i].NumSubsets << "\n";
            file << "BoundingBoxCenter: ("
                << meshes[i].BoundingBoxCenter.x << ", "
                << meshes[i].BoundingBoxCenter.y << ", "
                << meshes[i].BoundingBoxCenter.z << ")\n";
            file << "BoundingBoxExtents: ("
                << meshes[i].BoundingBoxExtents.x << ", "
                << meshes[i].BoundingBoxExtents.y << ", "
                << meshes[i].BoundingBoxExtents.z << ")\n\n";
        }

        // Write subsets
        for (UINT i = 0; i < header.NumTotalSubsets; ++i) {
            file << "SDKMESH_SUBSET [" << i << "]:\n";
            file << "Name: " << subsets[i].Name << "\n";
            file << "MaterialID: " << subsets[i].MaterialID << "\n";
            file << "PrimitiveType: " << subsets[i].PrimitiveType << "\n";
            file << "IndexStart: " << subsets[i].IndexStart << "\n";
            file << "IndexCount: " << subsets[i].IndexCount << "\n";
            file << "VertexStart: " << subsets[i].VertexStart << "\n";
            file << "VertexCount: " << subsets[i].VertexCount << "\n\n";
        }

        // Write frames
        for (UINT i = 0; i < header.NumFrames; ++i) {
            file << "SDKMESH_FRAME [" << i << "]:\n";
            file << "Name: " << frames[i].Name << "\n";
            file << "Mesh: " << frames[i].Mesh << "\n";
            file << "ParentFrame: " << frames[i].ParentFrame << "\n";
            file << "ChildFrame: " << frames[i].ChildFrame << "\n";
            file << "SiblingFrame: " << frames[i].SiblingFrame << "\n";
            file << "Matrix: [ ";
            for (int m = 0; m < 4; ++m) {
                for (int n = 0; n < 4; ++n) {
                    file << frames[i].Matrix.m[m][n] << " ";
                }
            }
            file << "]\n\n";
        }
        ...
        for (UINT i = 0; i < header.NumMaterials; ++i) {
            file << "SDKMESH_MATERIAL [" << i << "]:\n";
            file << "Name: " << materials[i].Name << "\n";
            file << "MaterialInstancePath: " << materials[i].MaterialInstancePath << "\n";
            file << "DiffuseTexture: " << materials[i].DiffuseTexture << "\n";
            file << "NormalTexture: " << materials[i].NormalTexture << "\n";
            file << "SpecularTexture: " << materials[i].SpecularTexture << "\n";
            file << "Diffuse: ("
                << materials[i].Diffuse.x << ", "
                << materials[i].Diffuse.y << ", "
                << materials[i].Diffuse.z << ", "
                << materials[i].Diffuse.w << ")\n";
            file << "Ambient: ("
                << materials[i].Ambient.x << ", "
                << materials[i].Ambient.y << ", "
                << materials[i].Ambient.z << ", "
                << materials[i].Ambient.w << ")\n\n";
        }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    void WriteSDKAnimationDataToTextFile( SDKANIMATION_DATA* animationData,
        ...
        const std::string& filename)
    {
        std::ofstream file(filename, std::ios::app);  // Append mode to add to existing file
        if (!file.is_open()) {
            std::cerr << "Failed to open file for writing: " << filename << std::endl;
            return;
        }
            ...
            std::cout << "SDKMESH data written to text file: " << filename << std::endl;
            file << "SDKANIMATION_DATA:\n";
            for (UINT i = 0; i < numKeys; ++i) {
                file << "Keyframe [" << i << "]:\n";
                file << "Translation: (" << animationData[i].Translation.x << ", "
                    << animationData[i].Translation.y << ", "
                    << animationData[i].Translation.z << ")\n";
                file << "Orientation: (" << animationData[i].Orientation.x << ", "
                    << animationData[i].Orientation.y << ", "
                    << animationData[i].Orientation.z << ", "
                    << animationData[i].Orientation.w << ")\n";
                file << "Scaling: (" << animationData[i].Scaling.x << ", "
                    << animationData[i].Scaling.y << ", "
                    << animationData[i].Scaling.z << ")\n\n";
            }

            file.close();
            std::cout << "SDKANIMATION_DATA written to text file: " << filename << std::endl;
